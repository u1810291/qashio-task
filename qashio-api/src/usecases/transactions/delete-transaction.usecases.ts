...
export class GetUserByEmail {
  ...
  async execute(data: Transaction): Promise<Transaction | null> {
    const isTransactionExists = await this.transactionRepository.getTransaction(data.id)
    ...
    this.logger.log('DeleteTransactionUseCases execute', 'Transaction has been deleted')
    return null
  }
}
